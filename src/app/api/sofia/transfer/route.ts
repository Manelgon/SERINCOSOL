...
import { supabaseAdminSecondary } from '@/lib/supabaseAdminSecondary';
...
export async function POST(request: Request) {
    try {
        ...
        const { data: sofiaRecord, error: fetchError } = await supabaseAdminSecondary
        ...
        const { error: deleteError } = await supabaseAdminSecondary
            .from('incidencias_serincobot')
            .delete()
            .eq('id', sofiaId);
